...
describe('ViewToggle', () => {
    ...
    it('"All Jobs" button has active class when viewMode is ALL', () => {
        ...
        expect(screen.getByRole('button', { name: 'All Jobs' })).toHaveClass('active');
        expect(screen.getByRole('button', { name: 'Saved Jobs' })).not.toHaveClass('active');
    });

    it('"Saved Jobs" button has active class when viewMode is SAVED', () => {
        renderToggle({ viewMode: VIEW_MODES.SAVED });
        expect(screen.getByRole('button', { name: 'Saved Jobs' })).toHaveClass('active');
        expect(screen.getByRole('button', { name: 'All Jobs' })).not.toHaveClass('active');
    });
