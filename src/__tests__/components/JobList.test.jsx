...
describe('JobList', () => {
    describe('loading state', () => {
        ...
        it('does not show the job list while loading', () => {
            renderList({ isLoading: true, jobs: [] });
            expect(screen.queryByRole('region', { name: /job listings/i })).not.toBeInTheDocument();
        });
    });
    ...
    describe('job cards', () => {
        ...
        it('renders the correct job titles', () => {
            renderList();
            expect(screen.getByRole('heading', { name: mockJobs[0].title })).toBeInTheDocument();
            expect(screen.getByRole('heading', { name: mockJobs[1].title })).toBeInTheDocument();
        });
    });
